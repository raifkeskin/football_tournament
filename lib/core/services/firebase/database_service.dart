...
import '../../../features/player/models/award.dart';
import '../../../features/tournament/models/league.dart';
import '../../../features/match/models/match.dart';
import '../../../features/player/models/player_stats.dart';
  ...
  Stream<List<GroupModel>> getGroups(String leagueId) {
    ...
    if (AppConfig.activeDatabase == DatabaseType.supabase) {
      try {
        ...
        AppConfig.sqlLogResult(table: 'groups', operation: 'STREAM', error: e);
  ...
  Future<List<League>> getTeamActiveTournaments(String teamId) async {
    ...
    if (AppConfig.activeDatabase == DatabaseType.supabase) {
      try {
        ...
        final leaguesRes = (await Supabase.instance.client
                .from('leagues')
                .select()
    ...
    if (AppConfig.activeDatabase == DatabaseType.supabase) {
      try {
        AppConfig.sqlLogStart(
          table: 'teams',
          operation: 'STREAM',
          filters: 'primaryKey=id | clientFilter=group_id=$gid',
        );
        return Supabase.instance.client
            .from('teams')
            .stream(primaryKey: ['id'])
  ...
  Future<void> updatePlayer({
    ...
    if (AppConfig.activeDatabase == DatabaseType.supabase) {
      try {
        ...
        await Supabase.instance.client.from('players').update(payload).eq('id', id);
        AppConfig.sqlLogResult(table: 'players', operation: 'UPDATE');
      } catch (e) {
    ...
    if (AppConfig.activeDatabase == DatabaseType.supabase) {
      try {
        AppConfig.sqlLogStart(
          table: 'matches',
          operation: 'STREAM',
          filters: 'primaryKey=id | clientFilter=tournament_id=$id OR league_id=$id',
        );
        return Supabase.instance.client
            .from('matches')
            .stream(primaryKey: ['id'])
            .map((rows) {
              return rows
                  .where((r) {
                    ...
                    final lid = (r['league_id'] ?? '').toString().trim();
                    return tid == id || lid == id;
                  })
                  .map((r) => {...Map<String, dynamic>.from(r), 'id': (r['id'] ?? '').toString()})
                  .toList();
            });
      } catch (e) {
