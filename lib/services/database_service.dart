...
class DatabaseService {
  ...
  Future<bool> isLeagueUnique({
    ...
    final byKeys = await _db
        .collection('leagues')
        .where('nameKey', isEqualTo: nameKey)
        .where('subtitleKey', isEqualTo: subtitleKey)
        .get();
    for (final d in byKeys.docs) {
  ...
  Future<void> _ensureDummyPlayersForTeam({
    ...
    final existing = await _getPlayersOnce(teamId);
    if (existing.length >= minCount) return;

    final existingNames = existing.map((e) => e.name.trim()).toSet();
    final firstNames = [
      'Ahmet',
      'Mehmet',
      'Mustafa',
    ...
    const preferredFeet = ['Sağ', 'Sol', 'Çift'];
    ...
    String pickName() {
      ...
      final fallback = 'Oyuncu ${existingNames.length + 1}';
      existingNames.add(fallback);
      return fallback;
    }

    final usedNumbers = existing
        .map((e) => int.tryParse((e.number ?? '').trim()))
        .whereType<int>()
        .toSet();
    ...
    final need = minCount - existing.length;
    for (var i = 0; i < need; i++) {
      ...
      final player = PlayerModel(
        ...
        teamId: teamId,
        name: pickName(),
        number: pickNumber().toString(),
        position: pos,
        preferredFoot: preferredFeet[random.nextInt(preferredFeet.length)],
        birthYear: 1988 + random.nextInt(16),
        goals: 0,
        assists: 0,
        yellowCards: 0,
        redCards: 0,
  ...
  Future<void> _assertPlayerUnique({
    ...
    final byInt = await _db
        .collection('players')
        .where('birthYear', isEqualTo: birthYear)
        .get();
    await scan(byInt);

    final byStr = await _db
        .collection('players')
        .where('birthYear', isEqualTo: birthYear.toString())
        .get();
    await scan(byStr);
  }

  Future<void> addPlayer(PlayerModel player) async {
    await _assertPlayerUnique(
      name: player.name,
      birthYear: player.birthYear,
    );
    await _db.collection('players').add({
      ...player.toMap(),
      'nameKey': StringUtils.normalizeTrKey(player.name),
    });
  }
  ...
  Future<void> updatePlayer({
    ...
    if (data.containsKey('name') || data.containsKey('birthYear')) {
      final doc = await _db.collection('players').doc(playerId).get();
      final existing = doc.data() ?? const <String, dynamic>{};
      final nextName = (data['name'] ?? existing['name'] ?? '').toString();
      final rawBirth = data.containsKey('birthYear')
          ? data['birthYear']
          : existing['birthYear'];
      int? nextBirth;
      if (rawBirth is num) nextBirth = rawBirth.toInt();
      if (rawBirth is String) nextBirth = int.tryParse(rawBirth.trim());
      await _assertPlayerUnique(
        name: nextName,
        birthYear: nextBirth,
      ...
      data = {
        ...data,
        'nameKey': StringUtils.normalizeTrKey(nextName),
      };
    }
  ...
  Stream<List<PlayerModel>> getPlayers(String teamId) {
    return _db
        .collection('players')
        .where('teamId', isEqualTo: teamId)
        .snapshots()
  ...
  Future<void> updateMatchLineup({
    ...
    await _db.collection('matches').doc(matchId).update({
      isHome ? 'homeLineup' : 'awayLineup': lineup.toMap(),
      'updatedAt': FieldValue.serverTimestamp(),
    });
  }
  ...
  Stream<List<MatchModel>> getMatchesByDate({
    ...
    Query<Map<String, dynamic>> q2 = _db
        ...
        .where('matchDate', isLessThan: Timestamp.fromDate(endLocal));

    if (leagueId != null) {
      q1 = q1.where('leagueId', isEqualTo: leagueId);
      q2 = q2.where('leagueId', isEqualTo: leagueId);
    }

    final controller = StreamController<List<MatchModel>>.broadcast();
    List<MatchModel> latest1 = const [];
    List<MatchModel> latest2 = const [];

    void emit() {
      final merged = <String, MatchModel>{};
      for (final m in latest1) {
        merged[m.id] = m;
      }
      for (final m in latest2) {
        merged[m.id] = m;
      }
    ...
    controller.onListen = () {
      ...
      sub2 = q2.snapshots().listen((snap) async {
        final list = <MatchModel>[];
        for (final doc in snap.docs) {
          final data = doc.data();
          final matchDateTs = data['matchDate'];
          if (matchDateTs is Timestamp) {
            final matchDate = matchDateTs.toDate();
            final computed =
                "${matchDate.year}-${matchDate.month.toString().padLeft(2, '0')}-${matchDate.day.toString().padLeft(2, '0')}";
            if ((data['dateString'] as String?) != computed) {
              data['dateString'] = computed;
              await doc.reference.update({'dateString': computed});
            }
          }
  ...
  Future<void> addMatchEvent(MatchEvent event) async {
    ...
    if (event.type == 'goal') {
      await _db.runTransaction((txn) async {
        ...
        txn.update(matchRef, {scoreField: currentScore + 1});
      });
    }

    Future<void> bumpPlayer(String name, Map<String, String> deltaField) async {
      final playerSnap = await _db
          .collection('players')
          .where('teamId', isEqualTo: event.teamId)
          .where('name', isEqualTo: name)
          .limit(1)
          .get();
      if (playerSnap.docs.isEmpty) return;
      final playerDoc = playerSnap.docs.first;
      final pData = playerDoc.data();

      final updates = <String, dynamic>{};
      for (final entry in deltaField.entries) {
        final field = entry.key;
        updates[field] = (pData[field] ?? 0) + 1;
      }
      if (updates.isNotEmpty) {
        await playerDoc.reference.update(updates);
      }
    }

    if (event.playerName.trim().isNotEmpty) {
      if (event.type == 'goal') {
        await bumpPlayer(event.playerName, {'goals': '1'});
        final assist = (event.assistPlayerName ?? '').trim();
        if (assist.isNotEmpty) {
          await bumpPlayer(assist, {'assists': '1'});
        }
      } else if (event.type == 'assist') {
        await bumpPlayer(event.playerName, {'assists': '1'});
      } else if (event.type == 'yellow_card') {
        await bumpPlayer(event.playerName, {'yellowCards': '1'});
      } else if (event.type == 'red_card') {
        await bumpPlayer(event.playerName, {'redCards': '1'});
      }
    }
  }
  ...
  // --- NEWS (HABERLER) ---
  Future<void> addNews(String content) async {
  ...
  Stream<QuerySnapshot> getNews({bool includeUnpublished = false}) {
    final query = _db.collection('news').orderBy('createdAt', descending: true);
