...
void main() {
  group('Standings Calculation Tests', () {
    ...
    test(
      'Teams should initialize with zero standings',
      () async {
        // Add teams
    ...
    test(
      'Standings should update correctly with completed matches',
      () async {
        // Add teams
        await firestore.collection('teams').doc(team1Id).set({
          'groupId': testGroupId,
    ...
    test(
      'Groups should be queryable with tournamentId filter',
      () async {
        // Add a group
        ...
        expect(
          (groupsSnapshot.docs.first.data()['name'] ?? ''),
          'Grup A',
        );
      },
    );
